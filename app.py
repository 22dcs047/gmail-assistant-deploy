...
Format as valid JSON only."""
    ...
    def _create_display_snippet(self, subject, snippet, body):
        """Create a smart 1-2 line summary for dashboard display"""
        # Use snippet first (Gmail's AI-generated summary), then body as fallback
        ...
        # Remove common email footers and unsubscribe text
        footer_patterns = [
            r'--.*
...
if __name__ == '__main__':
    ...
    app.run(debug=True)
        ...
        # Create intelligent summary based on email type
        subject_lower = subject.lower()
        
        if 'deadline' in subject_lower or 'registration' in subject_lower:
            # For deadline emails, extract key info
            if 'deadline' in text.lower():
                deadline_match = re.search(r'deadline[^.]*[.!]', text, re.IGNORECASE)
                if deadline_match:
                    return deadline_match.group(0).strip()
            return f"Registration deadline notice. {text[:80]}..."
        
        elif 'meeting' in subject_lower or 'invitation' in subject_lower:
            return f"Meeting invitation or schedule update. {text[:80]}..."
        
        elif 'security' in subject_lower or 'alert' in subject_lower:
            return f"Security notification. {text[:80]}..."
        
        elif any(word in subject_lower for word in ['opportunity', 'job', 'internship']):
            return f"Career opportunity notification. {text[:80]}..."
        
        else:
            # For general emails, try to get the most meaningful sentence
            sentences = text.split('.')
            meaningful_sentences = [s.strip() for s in sentences if len(s.strip()) > 15 and not s.strip().startswith('http')]
            
            if meaningful_sentences:
                # Take first meaningful sentence, max 100 chars
                summary = meaningful_sentences[0][:100]
                if len(meaningful_sentences[0]) > 100:
                    summary += "..."
                return summary
            else:
